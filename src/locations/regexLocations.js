...
function regexManualLocations(manualLocations, locations){
	Object.keys(manualLocations).forEach(zone => {
		if(!(zone in locations)){
			locations[zone] = {}
		}
		Object.keys(manualLocations[zone]).forEach(method => {
			locations[zone][method] = manualLocations[zone][method]
		})
	})
